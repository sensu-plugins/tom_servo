namespace :github do
  ...
  task :create_github_release do
    ...
    puts `curl -H "Authorization: token #{ github_token }" -d '{ "tag_name": "#{ spec.version }", "target_commitish": "#{ ENV['CI_COMMIT_ID'] }", "name": "#{ spec.version }", "body": "#{ ENV['CI_MESSAGE'] }" }' https://api.github.com/repos/sensu-plugins/#{ plugin }/releases` # rubocop:disable all
  end
